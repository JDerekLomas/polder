...
| 3 | Real login works (GitLab OAuth live) | 🟡 **auto-push verified end-to-end** on gitlab.com (glab authed as JDerekLomas, PAT seeded into the app's `gitlab.json`; create-repo + `oauth2:token` push smoke-tested OK). The "Log in with GitLab" OAuth button still needs an **Application ID** registered at gitlab.com user-settings/applications (admin-only via API → must be done in UI; redirect `http://127.0.0.1:4272/oauth/callback`, public/PKCE, scope `api`). For the pilot, push works now without it |
