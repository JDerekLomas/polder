...
| 5 | Publish yields a real **public** URL | 🟢 **done** — Scaleway Object Storage bucket `makemode-publish` (fr-par) created with static website hosting; app's `publish_to_eu` wired to it (creds fall back to the Scaleway keys). End-to-end `s3 sync → public URL` smoke-tested HTTP 200 at `https://makemode-publish.s3-website.fr-par.scw.cloud/<slug>/`. Same French vendor as inference = one DPA |
